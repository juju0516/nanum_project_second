...
<script type="text/javascript">
	...
	function goodsOne_Order_exec(goods_basket_idx, goods_idx, index) {
		var amt = document.getElementsByClassName("amt");
		var amount = amt[index].value;
		location.href="goods_basket_direct_sale.do?goods_basket_idx="+goods_basket_idx+"&goods_idx="+goods_idx+"&amount="+amount;
	}

	
 	/* 체크박스로 선택하여 주문하는 경우(1개일수도 , 2개 이상일 수도 있다.)*/
  	
 	function total_order_exec(f) {
 		alert('1212121212121212');
		var checkedItems = document.querySelectorAll('.goods_chk:checked');
 		if (checkedItems.length > 0) {	
 			alert(checkedItems.length)
 			//1. 1개이상 값이 체크 되어야 한다.
 			//2. 체크된 값만 보내져야 한다.... 다시하자!
/* 	 		var goodsBasket = []; // 체크된 굿즈 정보를 배열로 저장하자
		    ...
		    f.append(goodsBasket);
		    alert(goodsBasket.length) */
 		    // 폼을 서버로 제출한다. 
					...
					<c:forEach var="k" items="${basket_goodslist}" varStatus="vs"> 
						<tr id="list">
							...
							<td><span class="subject">[굿즈] ${k.goods_name}</span><br>
								...
								<input type="button" class="correction_btn" value="수정" onclick="basket_amount_edit('${k.goods_basket_idx}','${k.goods_idx}','${vs.index}')" 
