				...
				<tbody>
					...
					<c:choose>
						<c:when test="${empty list}">
							<tr>
								<td colspan="5"><h2>정기 나눔 내역이 존재하지 않습니다</h2></td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach var="k" items="${list}" varStatus="vs">
								<tr class="list">
									...
									<td>[정기]${k.prj_name}</td>
									<td>차감액:${k.dnt_point}나눔</td>
									<td>${k.r_dnt_date.substring(0,10)}</td>
		...
		<section>
			<ol class="paging">
				...
				<c:choose>
					...
					<c:otherwise>
						<li><a
							href="member_regular_list.do?cPage=${paging.beginBlock-paging.pagePerBlock}">
								&lt; </a></li>
					</c:otherwise>
				</c:choose>
				...
				<c:forEach begin="${paging.beginBlock}" end="${paging.endBlock}"
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a href="member_regular_list.do?cPage=${k}">${k}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
